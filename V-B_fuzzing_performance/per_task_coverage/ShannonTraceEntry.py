...
def dump_trace_entries(fapi, filename, trace_entry_addrs):
    ...
    for cur, caddr in enumerate(trace_entry_addrs):
        ...
        if trace_entry is None:
            print("Could not retrieve TraceEntry at 0x%s" % caddr.toString())
            continue
...
def main():
    ...
    for cur, caddr in enumerate(trace_entry_addrs):
        ...
        try:
            ...
            symbol_name = "TE::%s::%s" % (base_file, message_str_fixed)
            # limit the length
    ...
    filename = "trace-entries_%s.txt" % currentProgram.getName()
    dump_trace_entries(fapi, filename, trace_entry_addrs); return
